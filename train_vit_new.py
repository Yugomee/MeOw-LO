...
def main(args):
    ...
    for epoch in tqdm(range(args.epochs), desc="Epochs"):
        ...
        save_path = os.path.join(args.output_dir, f'epoch_{epoch+1}_vit_model.pt')
        save_model(model, save_path, epoch + 1)

    final_save_path = os.path.join(args.output_dir, f'output_model_batch_vit_{datetime.now().strftime("%Y-%m-%d_%H-%M-%S")}.pt')
